...
__global__ void XW_kernel(float *in_X, float *W, float *out_X, int v_num, int in_dim, int out_dim)
{
    extern __shared__ float shared_mem[];
    float *shared_X = shared_mem;
    float *shared_W = shared_mem + in_dim;
    ...
    if (row < v_num && col < out_dim)
    {
        ...
        {
            shared_X[threadIdx.y * in_dim + k] = in_X[row * in_dim + k];
            shared_W[k * blockDim.x + threadIdx.x] = W[k * out_dim + col];
        }
        __syncthreads();

        for (int k = 0; k < in_dim; k++)
        {
            sum += shared_X[threadIdx.y * in_dim + k] * shared_W[k * blockDim.x + threadIdx.x];
        }
...
void XW(int in_dim, int out_dim, float *in_X, float *out_X, float *W)
{
    ...
    dim3 blocksPerGrid((out_dim + 31) / 32, (v_num + 31) / 32);
    size_t shared_memory_size = (in_dim + out_dim) * sizeof(float);
    XW_kernel<<<blocksPerGrid, threadsPerBlock, shared_memory_size>>>(d_in_X, d_W, d_out_X, v_num, in_dim, out_dim);
